...
// Mapper untuk data input Framingham
const framinghamInputMap = [
  ...
  { label: "Total Kolesterol", field: "framinghamTotalCholesterol", unit: "mg/dL" },
  { label: "Tekanan Darah Sistolik", field: "framinghamSystolicBp", unit: "mmHg" },
  { label: "Merokok", field: "framinghamIsSmoker", unit: "" },
  { label: "Mengkonsumsi Obat Hipertensi", field: "framinghamIsOnHypertensionTreatment", unit: "" },
];

// Mapper untuk hasil/output Framingham
const framinghamResultMap = [
    { label: "Estimasi Risiko CVD", field: "framinghamRiskPercentage", unit: "%" },
    { label: "Kategori Risiko", field: "framinghamRiskCategory", unit: "" },
    { label: "Estimasi Usia Vaskular", field: "framinghamVascularAge", unit: "tahun" },
];

const displayValue = (v: unknown): string =>
  v === null || v === undefined || String(v) === "" ? "-" : String(v);

export const FraminghamDocument: React.FC<{ data: FraminghamData }> = ({ data }) => {
  return (
      ...
      <View style={styles.body}>
        ...
        <View style={styles.table}>
          <View style={styles.tableRow}>
            ...
            <Text style={[styles.tableColHeader, { flex: 2 }, styles.tableColLast]}>SATUAN</Text>
          </View>
          {framinghamInputMap.map((item, index) => (
            <View key={index} style={[styles.tableRow, index === framinghamInputMap.length - 1 && styles.tableRowLast]}>
              <Text style={[styles.tableCol, { flex: 3 }]}>{item.label}</Text>
              <Text style={[styles.tableCol, { flex: 2 }]}>{displayValue(data[item.field])}</Text>
              <Text style={[styles.tableCol, { flex: 2 }, styles.tableColLast]}>{item.unit}</Text>
            </View>
          ))}
        </View>
        ...
        {/* Tabel Hasil */}
        <Text style={[styles.subTitle, { textAlign: 'left', marginBottom: 8 }]}>HASIL</Text>
        <View style={styles.table}>
           <View style={styles.tableRow}>
          ...
          {framinghamResultMap.map((item, index) => (
            <View key={index} style={[styles.tableRow, index === framinghamResultMap.length - 1 && styles.tableRowLast]}>
              <Text style={[styles.tableCol, { flex: 3 }]}>{item.label}</Text>
              <Text style={[styles.tableCol, { flex: 2 }]}>{displayValue(data[item.field])}</Text>
              <Text style={[styles.tableCol, { flex: 2 }, styles.tableColLast]}>{item.unit}</Text>
            </View>
