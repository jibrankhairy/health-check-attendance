...
export const McuReport = ({ patient }: McuReportProps) => {
  ...
  const examinationDate = new Date().toLocaleDateString("id-ID", { day: 'numeric', month: 'long', year: 'numeric' });
  const examinationTime = new Date().toLocaleTimeString("id-ID", { hour: '2-digit', minute: '2-digit' });
    ...
    <div className="bg-white p-8 rounded-lg shadow-md border max-w-4xl mx-auto font-sans">
      ...
      <div className="space-y-3 text-sm">
        ...
        <div className="flex">
          ...
          <p className="font-semibold">: {formatDateToIndonesian(patient.dob)} / {patient.age} TAHUN</p>
        </div>
        <div className="flex">
        ...
        <div className="flex">
          ...
          <p className="font-semibold">: {patient.address?.toUpperCase() || "BELUM DIISI"}</p>
        </div>
        <div className="flex">
          <p className="w-48 shrink-0 text-gray-600">NO. TELP</p>
          <p className="font-semibold">: {patient.phoneNumber || "BELUM DIISI"}</p>
        </div>
        <div className="flex">
        ...
        <div className="flex">
          ...
          <p className="font-semibold">: {patient.companyName?.toUpperCase() || "BELUM DIISI"}</p>
        </div>
      </div>
