...
export async function POST(request: Request) {
  try {
    ...
    for (const patientData of patients) {
        if (!patientData.nik || !patientData.fullName || !patientData.dob) continue;
        ...
        const existingPatientByNik = await prisma.patient.findFirst({ where: { patientId: patientData.nik } });
    ...
    for (const patient of createdPatients) {
        if (patient.email) {
            try {
                ...
                console.error(`Gagal mengirim email ke ${patient.email}:`, emailError);
            }
        }
    }
