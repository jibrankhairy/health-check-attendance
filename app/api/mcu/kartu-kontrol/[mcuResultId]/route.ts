...
export async function GET(
  ...
  try {
    ...
    if (!mcuResultId) {
      return new NextResponse("MCU Result ID tidak ditemukan.", {
        status: 400,
      });
    }
    ...
    if (!mcuResult || !mcuResult.patient) {
      return new NextResponse("Data MCU atau Pasien tidak ditemukan.", {
        status: 404,
      });
    }
    ...
    const safePatientData = {
      ...
      mcuPackage: Array.isArray(mcuResult.patient.mcuPackage)
        ? mcuResult.patient.mcuPackage
        : [],
    };
    ...
    const fileName = `kartu_kontrol_${safePatientData.fullName.replace(
      /\s/g,
      "_"
    )}.pdf`;
