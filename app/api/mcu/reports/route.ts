    ...
    export async function GET() {
      ...
      try {
        ...
        console.log(`[API DEBUG] Menemukan ${allRelevantResults.length} data dengan status IN_PROGRESS atau COMPLETED.`);
        ...
        const finalResults = allRelevantResults.filter(result => {
          ...
          const isAllCompleted = patientPackage.every(packageName => {
            ...
            if (!dbField) {
              console.log(`    - GAGAL: Paket "${packageName}" tidak ada di dalam mapping sistem.`);
              return false;
            }
            ...
            if (check) {
              console.log(`    - OK: Paket "${packageName}" statusnya adalah "${packageStatus}".`);
            } else {
              console.log(`    - GAGAL: Paket "${packageName}" statusnya "${packageStatus}", seharusnya "COMPLETED".`);
            }
            return check;
          });
