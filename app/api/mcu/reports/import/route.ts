...
// Helper untuk mengubah nama kolom Excel ke field Prisma
  ...
  for (const key of allKeys) {
    ...
    if (value !== null && value !== undefined && value !== "") {
      if (integerFields.has(key)) {
        ...
        // =================================================================
        // === PERUBAHAN: Semua kolom lain (termasuk 4 kolom baru) akan   ===
...
export async function POST(request: Request) {
  try {
    ...
    if (!companyId) {
      return NextResponse.json({ message: "ID Perusahaan tidak ditemukan." }, { status: 400 });
    }
    ...
    if (!jsonData[0].hasOwnProperty('nik')) {
      return NextResponse.json({ message: "Kolom 'nik' wajib ada di dalam file Excel." }, { status: 400 });
    }
    ...
    for (const [index, row] of jsonData.entries()) {
      ...
      try {
        ...
        if (!patient) {
          errors.push(`Baris ${rowNum}: Pasien dengan NIK ${nik} tidak ditemukan.`);
          continue;
        }

        const mcuResult = patient.mcuResults[0];
        if (!mcuResult) {
          errors.push(`Baris ${rowNum}: Laporan MCU untuk pasien NIK ${nik} tidak ditemukan.`);
          continue;
        }
        ...
        errors.push(`Baris ${rowNum} (NIK: ${nik}): Gagal diproses. ${errorMessage}`);
      }
    }
    ...
    return NextResponse.json({ message: "Gagal mengimpor data.", errors: [msg] }, { status: 500 });
  }
}