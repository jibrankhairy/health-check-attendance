// Lokasi File: app/api/mcu/reports/export-results/route.ts

import { NextResponse } from "next/server";
import { PrismaClient } from '@prisma/client';
import * as ExcelJS from "exceljs";

// Membuat instance PrismaClient langsung di dalam file
const prisma = new PrismaClient();

/**
 * Fungsi ini memastikan data yang masuk adalah sebuah objek, lalu langsung mengembalikannya.
 */
...
// --- BARU: Logika perhitungan FAS dari FasDocument.tsx direplikasi di sini ---
...
const fasQuestions: { id: FasId; reverse?: boolean }[] = [
  { id: "fas1" }, { id: "fas2" }, { id: "fas3" },
  { id: "fas4", reverse: true }, { id: "fas5" }, { id: "fas6" },
  { id: "fas7" }, { id: "fas8" }, { id: "fas9" },
  { id: "fas10", reverse: true },
];

const categoryExplanation: Record<Exclude<Category, "N/A">, string> = {
  FATIGUED: "Tidak dapat mengatasi kelelahan akibat aktivitas berat dimana kondisi kesehatan mengalami gangguan.",
  TIRED: "Rutin melakukan aktivitas yang berat dan kurang istirahat yang cukup.",
  "SLIGHTLY TIRED": "Mengalami sedikit kelelahan karena melakukan aktivitas yang cukup berat.",
  FIT: "Faktor terjadi lelah karena adanya aktivitas normal, hanya membutuhkan tidur yang cukup.",
};

const calculateFasResult = (answers?: FasAnswers | null): {
    fas_raw_total: number | null;
    fas_score_1_to_10: number | null;
    fas_category: Category;
    fas_explanation?: string;
} => {
    if (!answers || Object.keys(answers).length === 0) return { fas_raw_total: null, fas_score_1_to_10: null, category: "N/A" };
    ...
    if (answered < fasQuestions.length) return { fas_raw_total: null, fas_score_1_to_10: null, category: "N/A" };
    ...
    const explanation = category === "N/A" ? undefined : categoryExplanation[category];

    return { fas_raw_total: total, fas_score_1_to_10: score1to10, fas_category: category, fas_explanation: explanation };
};

// Handler untuk request GET ke endpoint ini
export async function GET(request: Request) {
  try {
    ...
    if (!companyId) {
      return NextResponse.json({ message: "Company ID wajib diisi" }, { status: 400 });
    }
    ...
    if (reports.length === 0) {
      return NextResponse.json({ message: "Tidak ada data form yang bisa diekspor untuk perusahaan ini." }, { status: 404 });
    }
    ...
    worksheet.columns = [
        ...
        // Hasil DASS
        { header: "dass_depression_score", key: "dass_depression_score", width: 22 },
        { header: "dass_depression_level", key: "dass_depression_level", width: 22 },
        ...
        { header: "memilikiRiwayatPenyakit", key: "memilikiRiwayatPenyakit", width: 25 },
        { header: "riwayatPenyakitDetail", key: "riwayatPenyakitDetail", width: 35 },
        { header: "pernahDioperasi", key: "pernahDioperasi", width: 25 },
        { header: "riwayatPenyakitKeluarga", key: "riwayatPenyakitKeluarga", width: 25 },
        { header: "riwayatPenyakitKeluargaDetail", key: "riwayatPenyakitKeluargaDetail", width: 35 },
        ...
        { header: "anemiaOD", key: "anemiaOD", width: 15 }, { header: "anemiaOS", key: "anemiaOS", width: 15 },
        { header: "ikterikOD", key: "ikterikOD", width: 15 }, { header: "ikterikOS", key: "ikterikOS", width: 15 },
        { header: "pupilOD", key: "pupilOD", width: 15 }, { header: "pupilOS", key: "pupilOS", width: 15 },
        { header: "refleksOD", key: "refleksOD", width: 15 }, { header: "refleksOS", key: "refleksOS", width: 15 },
        { header: "pupilDistance", key: "pupilDistance", width: 15 },
        { header: "kacamata", key: "kacamata", width: 15 },
        { header: "ukuranOD", key: "ukuranOD", width: 15 }, { header: "ukuranOS", key: "ukuranOS", width: 15 },
        { header: "visusOD", key: "visusOD", width: 15 }, { header: "visusOS", key: "visusOS", width: 15 },
        { header: "lapangPandang", key: "lapangPandang", width: 20 },
        { header: "ketajaman", key: "ketajaman", width: 15 },
        { header: "kemampuanPendengaranAD", key: "kemampuanPendengaranAD", width: 20 }, { header: "kemampuanPendengaranAS", key: "kemampuanPendengaranAS", width: 20 },
        { header: "telingaLuarAD", key: "telingaLuarAD", width: 20 }, { header: "telingaLuarAS", key: "telingaLuarAS", width: 20 },
        { header: "nyeriTekanAD", key: "nyeriTekanAD", width: 20 }, { header: "nyeriTekanAS", key: "nyeriTekanAS", width: 20 },
        { header: "serumenAD", key: "serumenAD", width: 15 }, { header: "serumenAS", key: "serumenAS", width: 15 },
        { header: "gendangAD", key: "gendangAD", width: 20 }, { header: "gendangAS", key: "gendangAS", width: 20 },
        ...
        { header: "hepar", key: "hepar", width: 20 }, { header: "lien", key: "lien", width: 20 },
    ...
    reports.forEach((report) => {
        ...
        const dassResult = report.dassTestAnswers && (report.dassTestAnswers as any).result 
            ? (report.dassTestAnswers as any).result 
            : {};
        
        const fasRawAnswers = report.fasTestAnswers ? (report.fasTestAnswers as any) : {};
    ...
    const fileName = `hasil-mcu-${companyId}-${new Date().toISOString().split('T')[0]}.xlsx`;
