...
const PetugasDashboardPage = () => {
  ...
  if (!user) {
    return <div className="p-8 text-center bg-slate-100 min-h-screen">Memuat data petugas...</div>;
  }
          ...
          <Card className="w-full max-w-2xl shadow-xl rounded-2xl overflow-hidden bg-white/90 backdrop-blur-md border border-white/20">
            <CardHeader className="bg-white/80 border-b border-white/20 p-6">
              <div className="flex flex-col items-center text-center space-y-4 sm:flex-row sm:space-y-0 sm:space-x-4 sm:text-left">
                ...
                <div>
                  <CardTitle className="text-xl sm:text-2xl font-bold text-slate-800">Selamat Datang, {user.fullName}!</CardTitle>
                  <CardDescription className="text-slate-600">Anda login sebagai Petugas.</CardDescription>
                </div>
              </div>
            </CardHeader>
            ...
            <CardContent className="p-6 md:p-8 space-y-8 md:space-y-10">
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-lg md:text-xl font-semibold text-slate-800">Langkah 1: Pilih Pos Anda</h3>
                </div>
                <Select onValueChange={setSelectedCheckPoint} value={selectedCheckPoint || undefined}>
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  <ScanLine className={selectedCheckPoint ? "text-green-600" : "text-slate-500"} size={24} />
          ...
          <div className="fixed inset-0 bg-black bg-opacity-90 flex flex-col items-center justify-center z-50 p-4">
            <div className="relative bg-black border-4 border-slate-700 p-2 rounded-2xl shadow-2xl w-full max-w-md">
              ...
              <div className="p-4 md:p-6 bg-slate-800 rounded-t-lg text-white text-center">
                ...
                <h2 className="text-xl md:text-2xl font-bold">{selectedCheckPoint}</h2>
              </div>
              ...
              <div id="qr-reader-container" className="w-full bg-slate-900 p-2"></div>
            </div>
          </div>
        )}
      </div>
