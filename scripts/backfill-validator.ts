...
async function main() {
  ...
  // 1. Ambil SEMUA data yang form fisiknya sudah terisi
  ...
  // 2. Filter secara manual di dalam script menggunakan JavaScript
  const resultsToUpdate = allResults.filter((result) => {
    const form = result.pemeriksaanFisikForm as any;
    // Hanya proses data yang BELUM punya 'fisikValidatorName'
    return typeof form === "object" && form !== null && !form.hasOwnProperty("fisikValidatorName");
  });


  if (resultsToUpdate.length === 0) {
    console.log("Tidak ada data lama yang perlu di-update. Semua sudah lengkap.");
    return;
  }
